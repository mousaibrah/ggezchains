...
func (k Keeper) MintOrBurnCoins(ctx sdk.Context, tradeData types.StoredTrade, coin sdk.Coin) (status string, err error) {
	...
	if tradeData.TradeType == types.Buy {
		errMintCoins := k.bankKeeper.MintCoins(ctx, types.ModuleName, sdk.NewCoins(coin))
		if errMintCoins != nil {
			return types.Failed, errMintCoins
		}

		errSendCoinsFromModuleToAccount := k.bankKeeper.SendCoinsFromModuleToAccount(ctx, types.ModuleName, receiverAddress, sdk.NewCoins(coin))
		...
		errSendCoinsFromAccountToModule := k.bankKeeper.SendCoinsFromAccountToModule(ctx, receiverAddress, types.ModuleName, sdk.NewCoins(coin))
...
func (k Keeper) IsAddressLinkedToValidator(stakingKeeper types.StakingKeeper, goCtx context.Context, address string) (isAddressLinkedToValidator bool) {
	...
	chainValidators, _ := stakingKeeper.GetAllValidators(ctx)
